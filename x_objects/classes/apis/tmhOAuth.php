...
*/
  private function create_timestamp() {
    ...
    //$container->log( xevent::debug, "$tag->event_format : timestamp is ".$this->config['timestamp'] . "( ". date("Y-m-d h:i:s",$this->config['timestamp']).")");
  }
...
*/
  private function prepare_url($url) {
    ...
    //$container->log( xevent::debug, "$tag->event_format : url = $this->url");
  }
...
*/
  function auto_fix_time_request($method, $url, $params=array(), $useauth=true, $multipart=false) {
	...
	if ( isset( $this->config['debug']) && $this->config['debug']){
  		echo "$tag->event_format : method=$method,url=$url params array below<br>\r\n";
		print_r($params);
 		echo "<br>\r\n$tag->event_format : params array above<br>\r\n";
	}
...
*/
  function url($request, $format='json') {
	...
	$url = implode('/', array(
  	...
  	if ( isset( $this->config['debug']) && $this->config['debug'])
  		echo "$tag->event_format : url = $url<br>\r\n";
